...
/*
 * Add position-background classes
 */

$background_positions = array_keys($config->get('position-background', array())); 
$position_background_classes = array();
for ($i = 0; $i < count($background_positions); $i++) {
    $background_class = $config->get("position-background.{$background_positions[$i]}.position-bg");
    $position_background_classes[$background_positions[$i]] = "tm-block-{$background_class}";
    if ($this['config']->get("position-background.{$background_positions[$i]}.position-full")) {
        $position_background_classes[$background_positions[$i]] .= " tm-block-full";
        if (isset($background_positions[$i-1])) $position_background_classes[$background_positions[$i-1]] .= " tm-block-full-padding";
    }
    if ($i == count($background_positions) - 1 || (isset($background_positions[$i+1]) && $background_class != $config->get("position-background.{$background_positions[$i+1]}.position-bg"))) {
        $position_background_classes[$background_positions[$i]] .= ' tm-block-padding';
    }
}
...
$body_config['sticky-navbar']   = (int) $config->get('sticky_navbar', 0);
